...
$streets = mysql_query('SELECT street.name FROM node LEFT JOIN way_nodes ON node.id = way_nodes.node_id LEFT JOIN way ON way_nodes.way_id = way.id LEFT JOIN street ON way.street_id = street.id WHERE node.id IN (' . implode(',', $street_nodes_id) . ') AND street.name != "' . mysql_real_escape_string($_REQUEST['street']) . '" AND street.name LIKE "%' . str_replace(array('%', '?'), array('\\%', '\\?'), mysql_real_escape_string($_REQUEST['street_intersection'])) . '%" LIMIT 20');
?>
<ul>
