...
function html_street_lookup($label, $name, $intersection = FALSE) {
	return '
		...
		($intersection === FALSE ? '' : 
                '<br /><label for="' . $name . '_intersection">' . html_utf8(i18n(ucfirst($label . ' intersection:'))) . '</label> <input type="text" name="' . $name . '_intersection" id="' . $name . '_intersection" value="' . (isset($_REQUEST[$name . '_intersection']) ? htmlentities($_REQUEST[$name . '_intersection'], ENT_QUOTES) : '') . '" />
